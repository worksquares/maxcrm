...
(
    ...
    workflow_execution_uuid UUID NOT NULL DEFAULT gen_random_uuid() UNIQUE,
    ...
    -- Constraints
    CONSTRAINT fk_workflow_exec_company FOREIGN KEY (company_id)
        REFERENCES public.companies(company_id) ON DELETE RESTRICT,
    CONSTRAINT fk_workflow_exec_workflow FOREIGN KEY (workflow_uuid)
        REFERENCES public.workflows(workflow_uuid) ON DELETE CASCADE,
...
CREATE INDEX idx_workflow_exec_company_id ON public.workflow_executions(company_id);
...
CREATE INDEX idx_workflow_exec_created ON public.workflow_executions(created_at DESC);

-- Comments
COMMENT ON TABLE public.workflow_executions IS 'Workflow execution history and logs';
COMMENT ON COLUMN public.workflow_executions.actions_executed IS 'Details of executed actions with results';
